...
public extension MutableCollection {
  func partitioning(by belongsInSecondPartition: (Element) throws -> Bool) rethrows -> ([Element], [Element]) {
    let (first, second): (SubSequence, SubSequence) = try partitioning(by: belongsInSecondPartition)
    return (Array(first), Array(second))
  }
  
  func partitioning(by belongsInSecondPartition: (Element) throws -> Bool) rethrows -> (SubSequence, SubSequence) {
